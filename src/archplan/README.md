...
ArchPlanLab (extends BaseWorld)
  ...
  │     │                            • 🏛 Mái = MÁI CHUẨN frustum chia tọa độ A–X (roof-lab + roof-preview WebGL + ops/ thư viện op Houdini-style: #1 resample.ts arc-length · #2 sweep.ts quét tiết diện parallel-transport · #3 copy-to-points.ts 2 tầng gridOnSurface→điểm+frame / copyToPoints→InstancedMesh; gốc bắt đầu mọi dạng mái — memory canonical-roof-base):
  ...
  │     │                            • 🟫 Nền (ground-lab): demo CHỐNG LẶP texture ground — ① texture BOMBING iq (hash ô → xoay 90°×k HOẶC tự do + offset + jitter scale per ô, trộn mép 4 ô; Seed đổi bố cục) ② MACRO noise tần thấp (Cường độ sáng/tối + LOANG ÚA nhuộm ấm từng vạt) ③ BẢNG TRỘN LỚP động: Nền chính (select, mặc định grass_o) + nút ＋ thêm lớp (tối đa 4 slot shader uTexL0–3, mask fbm seed riêng/slot), mỗi lớp select 1 trong 9 texture kho + Ngưỡng riêng + ✕ xóa; Mềm biên/Scale mask chung. Texture cache theo key, load lazy. Tick tắt từng kỹ thuật so sánh bệnh↔thuốc. Plane 12×12 ShaderMaterial.
