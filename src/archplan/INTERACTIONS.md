...
### 5d. SITE element — ĐÀN CÁ 🐟 (FishSchool / PondFish)

Đàn cá = **CON của hồ pond** (`WaterConfig.fishSchools[]` ↔ `_siteFish[]` cfg↔PondFish) — NHIỀU đàn/pond (chuỗi
bậc, mỗi đàn `tier`). InstancedMesh trong `siteGroup`, update(dt) mỗi frame ở `onUpdate` (vẫy GPU, wander CPU).
Vùng bơi = LÒNG HỒ thật (`bounds`) — KHÔNG vị trí/Move riêng (đi theo hồ chứa).

| Tương tác | Đàn cá làm sao |
|---|---|
| **👆 Focus** | click trúng ĐÀN CÁ 3D → drawer Ground → Water → type-tab Pond → instance hồ → tab **🐟 Cá** → **F-tab đàn** + flash tia-Y. Raycast **RIÊNG list mesh cá TRƯỚC hồ** (`_tryClickFish` đầu chain — cá bơi DƯỚI mặt nước trong suốt nên KHÔNG so distance: thấy cá là trúng). Anchor: `_tryClickFish` (lab) → `_navigateToWater(entry.water)` (mở hồ chứa) + `_previewFish(entry.cfg)`. |
